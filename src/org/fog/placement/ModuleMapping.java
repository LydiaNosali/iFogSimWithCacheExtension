...
public class ModuleMapping {
	...
	public void addModuleToDevice(String moduleName, String deviceName, int instanceCount){
		...
		if(!getModuleMapping().get(deviceName).containsKey(moduleName)) {
			...
			//System.out.println("deviceName " + deviceName + "moduleToHostMap " + getModuleToHostMap());
		}
	}
	...
	public static int getFogDevIdByName(String fogName){
		for(FogDevice fogdev: DataPlacement.fogDevices) {
			if(fogdev.getName().equals(fogName)) {
				return fogdev.getId();
			}
		}
		return -1;
	}
	public static String getFogDevNameById(int fogid){
		for(FogDevice fogdev: DataPlacement.fogDevices) {
			if(fogdev.getId()==(fogid)) {
				return fogdev.getName();
			}
		}
		return null;
	}
