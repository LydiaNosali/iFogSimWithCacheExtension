...
public class FogDevice extends PowerDatacenter {
	...
	private List<Tuple> storedData = new ArrayList<Tuple>();
...
//	}
	...
	protected void processOtherEvent(SimEvent ev) {
		//A l'apparition d'un evenement, le traitement relatif à cet evenement sera ici
		switch(ev.getTag()){
		
		//evenements relatifs au cache  
		case FogEvents.INITIALIZE_DATA_RETRIEVE:
			processInitiliazeDataRetrieve(ev);
			break;
		
		case FogEvents.SEND_DATA_TO_CONSUMER:
			processsend_data_to_consumer(ev);
			break;
			
		case FogEvents.STORE_DATA_CACHE:
			processstore_data_cache(ev);
			break;
		//////////////////////////////
	...
	private void processstore_data_cache(SimEvent ev) {
		// TODO Auto-generated method stub
		Tuple tuple = (Tuple) ev.getData();
		
		DataPlacement.fogcache.store(tuple, this.getId());
	}

	private Tuple getStoredTupleBytype(String tupleType) {
		for (Tuple tuple : storedData) {
			if(tuple.getTupleType().equals(tupleType))
				return tuple;
		}
		return null;
	}

	private void processsend_data_to_consumer(SimEvent ev) {
		// TODO Auto-generated method stub
		// TODO Auto-generated method stub
		////*System.out.println("Tuple Storage:"+ev.toString());
		Log.writeInLogFile(this.getName(), "Tuple Storage:"+ev.toString());
		//System.out.println((this.getName()+ "Tuple Storage:"+ev.toString()));
		...
		if(this.storedData.contains(tupleType)) {
			...
			DataPlacement.fogcache.retreive(tupleType, this.getId());
		}
	...
	private void processInitiliazeDataRetrieve(SimEvent ev) {
		// TODO Auto-generated method stub
		//System.out.println("processInitiliazeDataRetrieve started");
		//System.out.println(ev.toString());
		AppEdge edge = (AppEdge) ev.getData();
		String tupleType = edge.getTupleType();
		//System.out.println(DataPlacement.fogBroker.getId());
		DataPlacement.nb_request++;
		send(DataPlacement.fogBroker.getId(), 300, FogEvents.TUPLE_REQUEST, tupleType);
		
		send(this.getId(), 300, FogEvents.INITIALIZE_DATA_RETRIEVE, edge);

	}

	/*
	 * Send tuple placement to broker
	 */
	public void processTupleRequest(SimEvent ev) {
		//Log.writeInLogFile(this.getName(), "Tuple Storage:"+ev.toString());
		Tuple tuple = (Tuple) ev.getData();
	}
	/*
	 * Send periodic tuple requests to broker
	 */
	public void sendPeriodicRequest(SimEvent ev) {
		AppEdge edge = (AppEdge)ev.getData();
	...
	private void processTupleStorage(SimEvent ev) {
		...
		for(int cache : DataAllocation.dataPlacementCacheMap.get(tuple.getTupleType())) {
			float latency = BasisDelayMatrix.getFatestLink(cache, this.getId());
			Log.writeInLogFile(this.getName(),"Storing data in cache");
			//System.out.println("Storing data in cache");
			send(cache, latency, FogEvents.STORE_DATA_CACHE, tuple);
...
//		}
	...
	protected void processTupleProcess(SimEvent ev){
		...
		if(appToModulesMap.get(tuple.getAppId()).contains(tuple.getDestModuleName().get(0))){
			...
			//System.out.println("destDevId:"+destDevId);
	...
	protected void processModuleArrival(SimEvent ev){
		...
		//System.out.println("Hello 2 "+appToModulesMap.get(appId).add(module.getName()));
